...
export default class Nav extends React.Component {
  ...
  render() {
    ...
    if (this.state.active) {
      ...
      links.push(<li key={2}><Link to="/devices"><i className="fa fa-key fa-cube" /> Devices</Link></li>);
      links.push(<li key={3}><a href="#" onClick={this.logout}><i className="fa fa-key fa-unlock" /> {this.state.user.username}</a></li>);
    } else {
      links.push(<li key={10}><Link to="/register"><i className="fa fa-user fa-fw" /> Register</Link></li>);
      links.push(<li key={11}><Link to="/login"><i className="fa fa-key fa-lock" /> Login</Link></li>);
    }
